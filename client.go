...
func (c *Client) CreateUpload(u *Upload, remoteSize int64, partial bool, meta map[string]string) (response *http.Response, err error) {
	...
	//todo 从metadata中提取mapping 放到请求头里面
	//这个请求头的内容是一个映射路径 上传完后 或者 concatenate后 会根据映射 把完整文件复制到指定位置
	mapping, ok := meta["Upload-Copy-Path"]
	if ok {
		req.Header.Set("Upload-Copy-Path", mapping)
	}
	switch {
